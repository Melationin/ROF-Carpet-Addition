...
import static com.carpet.rof.commands.SearchCommand.executeCommand;
...
public class SafeTickCommand {
    ...
    @QuickTranslations(
            name = "安全加速",
            description = "添加了tick的子命令safe rate , 可以在无生存真人时自动加速"
    )
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        ...
        n1 = n1.then(literal("rate").then(argument("rate", IntegerArgumentType.integer()).executes((context -> {
            ...
            if(SafeTickRate == 20) context.getSource().sendFeedback(()-> Text.of("Set safe tick rate to "+commandSafeTick),true);
            return  0;
        }))));
